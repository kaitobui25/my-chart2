...
export class BinanceHistoryCache {
  ...
  private async readWithDeadline(read: (signal: AbortSignal) => Promise<Candle[]>): Promise<Candle[]> {
    const controller = new AbortController();
    const timeout = globalThis.setTimeout(() => controller.abort(), this.readTimeoutMs);
    try {
      const candles = await read(controller.signal);
      return controller.signal.aborted ? [] : candles;
    } catch {
      return [];
    } finally {
      globalThis.clearTimeout(timeout);
